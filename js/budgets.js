...
function loadBudgets() {
    ...
    try {
        ...
        app.fillMonthOptions('budgetMonth', false);
        app.fillYearOptions('budgetYear', false);
        ...
        console.log('Budgets page loaded successfully');
    } catch (error) {
        console.error('Error loading budgets page:', error);
        app.showNotification('Error loading budgets page', 'error');
    }
}
...
function updateBudgetsDisplay() {
    ...
    if (currentBudgets.length === 0) {
        ...
        if (addFirstBudgetBtn) {
            addFirstBudgetBtn.addEventListener('click', addNewBudget);
        }
        return;
    }
    ...
    currentBudgets.forEach((budget, index) => {
        // Cari kategori
        ...
        html += `
            <div class="budget-card fade-in-up" style="animation-delay: ${index * 0.1}s">
...
function editBudget(id) {
    ...
    if (!budget) {
        app.showNotification('Budget tidak ditemukan', 'error');
        return;
    }
...
function deleteBudget(id) {
    ...
    app.showNotification('Budget berhasil dihapus', 'success');
}
...
function saveBudget() {
    ...
    // Validasi tambahan
    if (amount <= 0) {
        app.showNotification('Jumlah budget harus lebih dari 0', 'error');
        return;
    }
...
function initBudgetsEventListeners() {
    console.log('Initializing budgets event listeners...');
    
    // Tombol tambah budget
    const addBudgetBtn = document.getElementById('addBudgetBtn');
    if (addBudgetBtn) {
        addBudgetBtn.addEventListener('click', addNewBudget);
    }
